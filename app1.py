...
def check_non_symmetry(matrix):
    if  (np.array(matrix) == np.array(matrix).T).all():
        raise ValueError("Error: The matrix is symmetric.")
    return True, None
    ...
    try:
        ...
        if matrix_type == 'dense':
            ...
            <div class="container mt-5">
                <h2>Result after Gauss-Jordan Elimination:</h2>
    ...
    try:
        ...
        if matrix_type == 'dense':
            ...
            <div class="container mt-5">
                ...
                <h2>Lower Triangular Matrix L:</h2>
                <table class="table table-bordered">
                    {% for row in L %}
                        <tr>
                            {% for elem in row %}
                ...
                <h2>Upper Triangular Matrix U:</h2>
                <table class="table table-bordered">
                    {% for row in U %}
                        <tr>
                            {% for elem in row %}
...
def process_matrix(matrixx,b):
    ...
    for k in range(n-1):
        for i in range(k + 1, n):
            ...
            if matrix[k][k] == 0:
                raise ValueError("Division by zero: Pivot element is zero.")
        ...
        if(check_non_symmetry(matrix)):
          if matrix_type == 'dense':
           result,x = gauss_with_partial_pivoting(matrix,vector)
            ...
            raise ValueError("Matrix should not be symetric.")
        
        
        
        result_html = '<div class="container mt-3"><h2>Resulting Matrix:</h2><div class="row">'
        if matrix_type == 'bande':
            result_html += f'<h2>Value of m: {m} </h2>'
        result_html += f'<h2>Value of x: {x}</h2>'
        for row in result:
            result_html += '<div class="col-12 d-flex justify-content-center">'
            for elem in row:
                result_html += f'<div class="p-2 border border-secondary rounded text-center matrix-cell">{elem:.2f}</div>'
            result_html += '</div>'
        result_html += '</div></div>'
    except ValueError as e:
...
def cholesky_decompose():
    try:
        ...
        if not is_symmetric(matrix):
            raise ValueError("Matrix is not symmetric. Cholesky decomposition is only applicable to symmetric positive definite matrices.")
        m = find_bandwidth(matrix)
        if matrix_type == 'dense':
        ...
        result_html = '<div class="container mt-3"><h2>Cholesky Decomposition:</h2><div class="row">'
        if matrix_type == 'bande':
            result_html += f'<h2>Value of m: {m}</h2>'
        ...
        result_html = f'<div class="container mt-3"><h2>Error:</h2><p>{str(e)}</p></div>'  
...
def cholesky_decomposition(matrix, b):
    if not is_positive_definite(matrix):
        raise ValueError("Matrix is not positive definite. Please ensure that all leading principal minors are positive.")
    ...
    if request.method == 'POST':
        try:
            ...
            if not is_upper_triangular(A):
                raise ValueError("Please enter an upper triangular matrix.")
...
def indextriangInfa():
    if request.method == 'POST':
        ...
        if not is_lower_triangular(A):
            return render_template('resotriangInf.html', solution=None, error="Please enter a lower triangular matrix.")
...
def resodemiinfa():
    if request.method == 'POST':
        try:
            ...
            for i in range(size):
                ...
                for j in range(size):
                    ...
                    if element_name in request.form:
                        ...
                        raise ValueError("Please fill in all matrix elements.")
            ...
            if not is_band:
                raise ValueError("Please enter a lower band matrix.")
...
def resodemisupa():
    if request.method == 'POST':
        try:
            ...
            if not is_band_test:
                raise ValueError("Please enter an upper band matrix.")
    ...
    if request.method == 'POST':
        try:
            ...
            if is_band_test:
                ...
                if determinant < 0:
                    return jsonify({'error': 'Determinant is negative. Cannot proceed with multiplication.'})
                ...
                return jsonify({'error': 'Matrix A does not meet the required band conditions.'})
...
def bandedemibandinf():
    try:
        ...
        if is_lower1 and is_band1:
            ...
            return jsonify({'error': 'Matrices do not meet the required band conditions.'})
...
def matrix_vector_multiplication(matrix, vector):
    if matrix.shape[1] != len(vector):
        raise ValueError("Matrix and vector have incompatible dimensions for multiplication")
...
def matrix_vector_multiply():
    ...
    try:
        matrix_rows = int(request.form['matrixRows'])
        matrix_cols = int(request.form['matrixCols'])
        vector_size = int(request.form['vectorSize'])

        # Récupérer les valeurs de la matrice
        matrix_values = []
        for i in range(matrix_rows):
            row_values = [float(request.form[f'matrix[{i}][{j}]']) for j in range(matrix_cols)]
...
def matrix_inf_vector_multiplication(matrix, vector):
    n = len(vector)
    if n != len(matrix):
        raise ValueError("Vector and matrix have different dimensions")
    result = np.zeros(n)
...
def infvecteurapp(): 
    try:
        ...
        for i in range(matrix_size):
             for j in range(i + 1):  # Adjusted loop condition here
                key = f'matrix[{i}][{j}]'
...
def matrix_sup_vector_multiplication(matrice, vecteur):
    if len(matrice) != len(vecteur):
        raise ValueError("Les dimensions de la matrice et du vecteur ne correspondent pas")
    ...
    try:
        ...
        for i in range(1, matrix_size + 1):
            for j in range(1, matrix_size + 1):
                key = f'matrix[{i}][{j}]'
                matrix_values[i-1, j-1] = float(request.form[key])

        # Retrieve vector values
        vector_values = [float(request.form[f'vector[{i}]']) for i in range(1, vector_size + 1)]
...
def demisupvecteurapp():
    try:
        ...
        if is_band:
            ...
            return jsonify({'error': 'Matrix is not an upper band matrix with the specified bandwidth.'})
...
def solvegs():
    try:
        ...
        determinant = calculate_determinant(A)
        if determinant <= 0:
            return jsonify({'error': 'Determinant of matrix A is zero. Cannot proceed with the Gauss-Seidel method.'})
          ...
          # Calculate eigenvalues of G with a threshold
           ...
           # Check if the maximum eigenvalue is strictly less than 1
            ...
            if max_eigenvalue_G < 1 :  
               if iteration_test:
                  msg = "Matrix is not dominante ρ(G) =",max_eigenvalue_G,"< 1 Then The Gauss-Seidel method converges."
               ...
               msg ="Matrix is not dominante ρ(G) =",max_eigenvalue_G,"> 1 Then The Gauss-Seidel method diverges."
               return jsonify({"msg":msg,'result': None})
        else: 
            if iteration_test:
                  msg = "Matrix is dominante Then The Gauss-Seidel method converges."
                  result = gauss_seidel_method_iteration(A, b, iteration)
...
def solvejacobi():
    try:
        ...
        determinant = calculate_determinant(A)
        if determinant <= 0:
            return jsonify({'error': 'Determinant of matrix A is zero. Cannot proceed with the Jacobi method.'})
        ...
        if not is_diagonally_dominant(A):
            ...
            if max_eigenvalue_J < 1:
                if iteration_test:
                    msg = f"Matrix is not dominant ρ(J) = {max_eigenvalue_J:.1f} < 1. Jacobi method converges."
                ...
                msg = f"Matrix is not dominant ρ(J) = {max_eigenvalue_J:.1f} > 1. Jacobi method diverges."
                return jsonify({"msg": msg, 'result': None})
        else:
            if iteration_test:
                msg = "Matrix is dominant. Jacobi method converges."
                result = jacobi_method_iteration(A, b, iteration)
            else:
                msg = "Matrix is dominant. Jacobi method converges."
                result = jacobi_method_epsilon(A, b)
